...
//        int netWorkType = NetworkUtil.getNetworkType(context).value;
        int netWorkType = NetworkUtil.getNetWorkClass(context);
        String netType = "";

        //System.out.println("----------------->");
        if (isWifi(context)) {
            //System.out.println("netType = \"WIFI\"");
            return netType = "WIFI";
        }
        ...
        switch (netWorkType) {
            ...
            case NETWORKTYPE_3G:
                netType = "3G";
                //mTextView.setText("当前网络为3G移动网络,信号强度为：" + dianxing);
                break;
            case NETWORKTYPE_4G:
                netType = "4G";
                //mTextView.setText("当前网络为4G移动网络,信号强度为：" + dbm);
                break;
    ...
    public static int getConnectedTypeINT(Context context) {
        NetworkInfo net = getConnectivityManager(context).getActiveNetworkInfo();
        if (net != null) {
            //Log.i("===", "NetworkInfo: " + net.toString());
            return net.getType();
        }
        // Log.i("===", "NetworkInfo: " + net.toString());
        return -1;
...
//        String strSubTypeName = networkInfo.getSubtypeName();
        ...
        switch (type) {
            ...
            case ConnectivityManager.TYPE_MOBILE_SUPL:
                int teleType = getNetWorkClass(context);
                switch (teleType){
                    case 2:
